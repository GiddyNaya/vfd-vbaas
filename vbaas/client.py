...
class VBaaSClient:
    ...
    ENVIRONMENTS = {
        "test": {
            ...
            "bills_base_url": "https://api-devapps.vfdbank.systems/vtech-bills/api/v2/billspaymentstore",
        },
        "live": {
            "auth_base_url": "https://api-apps.vfdbank.systems/vfd-tech/baas-portal/v1.1",
            "bills_base_url": "https://api-apps.vfdbank.systems/vtech-bills/api/v2/billspaymentstore",
        }
    }
    ...
    def __init__(
        ...
        if environment not in self.ENVIRONMENTS:
            raise ConfigurationError(f"Invalid environment: {environment}. Must be 'test' or 'live'")
    ...
    def _get_access_token(self) -> str:
        ...
        try:
            ...
            if data.get("status") != "00":
                raise AuthenticationError(f"Authentication failed: {data.get('message', 'Unknown error')}")
    ...
    def _make_request(
        ...
        try:
            if method.upper() == "GET":
                ...
                response = self.session.post(url, json=data, params=params, headers=headers, timeout=self.timeout)
            else:
    ...
    def _validate_response(self, response: Dict[str, Any], success_status: str = "00") -> Dict[str, Any]:
    ...
    def get_biller_items(self, biller_id: str, division_id: str, product_id: str) -> List[BillerItem]:
        """
    ...
    def pay_bill(
        ...
        Returns:
            Payment response
        """
        if not all([customer_id, amount, division, payment_item, product_id, biller_id, reference, phone_number]):
